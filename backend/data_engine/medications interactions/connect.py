...
medications = pd.read_csv(os.path.join(current_folder, "dci.csv"), sep=",")
...
medication_lookup = {
    normalize(name): name 
    for name in medications["name"].str.strip()
}
...
with open(os.path.join(current_folder, "interactions_valid2.json"), "w", encoding="utf-8") as f:
