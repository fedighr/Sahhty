...
GROQ_API_KEY = "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"
...
with open(os.path.join(current_folder, "DCI_informations_lecrat.json"), "r", encoding="utf-8") as f:
    entries = json.load(f)

try:
    with open(os.path.join(current_folder, "classifications.json"), "r", encoding="utf-8") as f:
...
with open(os.path.join(current_folder, "classifications.json"), "w", encoding="utf-8") as f:
    json.dump(classifications, f, ensure_ascii=False, indent=2)

if failed:
    with open(os.path.join(current_folder, "classification_failed.txt"), "w", encoding="utf-8") as f:
        for dci in failed:
            f.write(f"{dci}\n")
