...
class PregnancyService:
    ...
    def getCurrentPregnancy(patient_id):
        try:
            ...
            pregnancy = Pregnancy.objects.filter(patient_id=patient_id, end_date__isnull=True, start_date__gte=nine_months_ago).first()
            if pregnancy:
                serializer = PregnancySerializer(pregnancy)
                return {'data': {'success': True, 'pregnancy': serializer.data}, 'status': 200}
            return {'data': {'success': False, 'message': 'No current pregnancy found for this patient'}, 'status': 404}
