...
EXCEL_FILE_PATH = ROOT_DIR / "Paramètres_original_extended.xlsx"
DATASET_PATH = BASE_DIR / "pregnancy_dataset.csv"
REPORT_PATH = BASE_DIR / "model_comparison_report_V2.pdf"
...
def parse_excel_number(value):
    if pd.isna(value):
        raise ValueError("Missing numeric value in Excel")
    text = str(value).strip().replace("_x000d_", "").replace("\r", "").replace("\n", "")
    text = text.replace(",", ".")
    return float(text)
        ...
        features.append(
            {
                "age": float(row["Age"]),
                "bmi": parse_bmi(row),
                "glucose": parse_excel_number(row["Glycémie à jeun g/L"]) * 100,
...
def evaluate_models():
    label_encoder = load_pickle(BASE_DIR / "label_encoder_V2.pkl")
    preprocess_config = load_pickle(BASE_DIR / "preprocess_config_V2.pkl")
    heart_rate_median = load_pickle(BASE_DIR / "heart_rate_median_V2.pkl")
    class_names = list(label_encoder.classes_)
    feature_columns = preprocess_config["feature_columns"]

    models = {
        "Random Forest": load_pickle(BASE_DIR / "pregnancy_rf_classifier_V2.pkl"),
        "Logistic Regression": load_pickle(BASE_DIR / "logistic_classifier_V2.pkl"),
        "SVM": load_pickle(BASE_DIR / "svm_classifier_V2.pkl"),
        "XGBoost": load_pickle(BASE_DIR / "xgboost_classifier_V2.pkl"),
    }
